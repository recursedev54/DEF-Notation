...
sounds = {
    ...
    'P': create_sound(0, 0.075, noise=True), # Pedal hi-hat: medium noise
...
def play_def_notation():
    ...
    hihat_sequence = notation.replace('|', '').split()
    tempo = int(tempo_entry.get())
    beat_duration = 60 / tempo / 4  # Each symbol represents a 16th note

    full_sequence = np.zeros(int(44100 * beat_duration * len(hihat_sequence)))
    
    for i, hihat in enumerate(hihat_sequence):
        if hihat in sounds:
...
# Create main window
root = tk.Tk()
root.title("DEF Notation Player (Hi-Hat Pattern)")
